...
local function decode_(fileContents)
    ...
    if type(j) ~= "table" then
        ccerror("[qeeplay.api.GameState] ERR, decode_() invalid contents")
    ...
    if testHash ~= hash then
        ccerror("[qeeplay.api.GameState] ERR, decode_() hash miss match")
    ...
    if type(values) ~= "table" then
        ccerror("[qeeplay.api.GameState] ERR, decode_() invalid state data")
        return {errorCode = M.ERROR_INVALID_FILE_CONTENTS}
    end
...
function M.init(eventListener_, stateFilename_, secretKey_)
    if type(eventListener_) ~= "function" then
        ccerror("[qeeplay.api.GameState] ERR, init() invalid eventListener")
        return false
    end
...
function M.load()
    ...
    if not io.exists(filename) then
        ccwarning("[qeeplay.api.GameState] load() file \"%s\" not found", filename)
        return eventListener({name = "load", errorCode = M.ERROR_STATE_FILE_NOT_FOUND})
    end

    local contents = io.readfile(filename)
    ccwarning("[qeeplay.api.GameState] load() get values from \"%s\"", filename)
    ...
    if secretKey and isEncodedContents_(contents) then
        ...
        if type(values) ~= "table" then
            ccerror("[qeeplay.api.GameState] ERR, load() invalid data")
...
function M.save(newValues)
    ...
    if type(values) ~= "table" then
        ccerror("[qeeplay.api.GameState] ERR, save() listener return invalid data")
        return false
    end
    ...
    if secretKey then
        ...
        local json = json.encode(values)
        if type(json) == "string" then
            ret = io.writefile(filename, json)
        end
    end

    ccwarning("[qeeplay.api.GameState] save() update file \"%s\"", filename)
    return ret
end
